...
public class DTLSSessionManager
{
    ...
    private readonly ILogger<DTLSSessionManager> logger;
    private readonly TlsPskIdentityManager pskManager = new PskManager();

    public DTLSSessionManager(ushort port, TimeSpan sessionTimeout, IMemoryCache cache, IServiceProvider serviceProvider)
    {
        ...
        this.logger = serviceProvider.GetRequiredService<ILogger<DTLSSessionManager>>();
    }

    public event EventHandler<ReceivedAuthenticatedPacketEventArgs>? ReceivedData;
    ...
    private void SessionReceivedData(object? sender, EventArgs args)
    {
        ...
        this.ReceivedData?.Invoke(this, new ReceivedAuthenticatedPacketEventArgs()
        {
