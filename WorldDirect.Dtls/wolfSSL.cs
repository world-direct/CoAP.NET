...
namespace WorldDirect.Dtls {
    ...
    public class wolfssl
    {
        ...
        [DllImport(wolfssl_dll, CallingConvention = CallingConvention.Cdecl)]
        private extern static IntPtr wolfSSL_get_peer_certificate(IntPtr ssl);
        ...
        [DllImport(wolfssl_dll, CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl)]
        private extern static int wolfSSL_dtls_got_timeout(IntPtr ssl);
        ...
        internal static DTLS_con get_dtls_fd(IntPtr ssl)
        {
            try
            {
                IntPtr ptr;
        ...
        public static bool dtls_got_timeout(IntPtr ssl)
        {
            if (ssl == IntPtr.Zero)
                return false;
            ...
            try
            {
                ...
                if (sslCtx == IntPtr.Zero)
                {
                    log(ERROR_LOG, "wolfssl dtls got_timeout error");
                    return false;
                }

                return wolfSSL_dtls_got_timeout(sslCtx) != SUCCESS;
            }
            catch (Exception e)
            {
                log(ERROR_LOG, "wolfssl get version error " + e.ToString());
        ...
        public static string get_error(int errorCode)
        {
            int err;
            StringBuilder err_name;
            err_name = new StringBuilder(new String(' ', 80));
            wolfSSL_ERR_error_string((uint)errorCode, err_name);

            return err_name.ToString();
        }

        public static int get_error_int(IntPtr ssl)
        {
            if (ssl == IntPtr.Zero)
            ...
            return wolfSSL_get_error(sslCtx, 0);
        }
        ...
        public static X509 get_peer_certificate(IntPtr ssl)
        {
            try
            {
                IntPtr local_ssl = unwrap_ssl(ssl);
                if (local_ssl == IntPtr.Zero)
                {
                    throw new ArgumentNullException("Cant load peer certificate of null", nameof(ssl));
                }
                IntPtr x509 = wolfSSL_get_peer_certificate(local_ssl);
                if (x509 != IntPtr.Zero)
                {
                    return new X509(x509, true);
                }

                throw new InvalidOperationException("Cant load peer certificate from peer");
            }
            catch (Exception e)
            {
